...
{ 
   ...
   switch (i)
   {
	 ...
	 // get rid of operations by manually calculating values
}
...
static uint8 huffDecode(const HuffTable* pHuffTable, const uint8* pHuffVal)
{
   ...
   while (1)
   {
		  if (i == 16) 
		 	...
		 	// AD Opt
			// huffDecode_ctr++; // variable count not used and not needed
...
static uint8 decodeNextMCU(void)
{
   ...
   for (mcuBlock = 0; mcuBlock < gMaxBlocksPerMCU; mcuBlock++) {
      ...
      compACTab = gCompACTab[componentID];
		...
		// Calculate ternary operation outside of loop
		HuffTable* table = compACTab ? &gHuffTab3 : &gHuffTab2;
		uint8* val = compACTab ? gHuffVal3 : gHuffVal2;
      if (gReduce) {
         // Decode, but throw out the AC coefficients in reduce mode.
         for (k = 1; k < 64; k++) {
           s = huffDecode(table, val); // where ternary operation was originally
					 numExtraBits = s & 0xF;
            if (numExtraBits)
