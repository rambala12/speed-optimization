...
#include "region.h"
#include "profile.h"
//#include <stdlib.h> // for quick sort function
...
// 	int a_val = *(const unsigned int*)a;
// 	int b_val = *(const unsigned int*)b;
// 	a_val = RegionCount[a_val];
// 	b_val = RegionCount[b_val];
...
// change bubble sort alg to quick sort
void Sort_Profile_Regions(void) {
	...
	// QUICK SORT
	// qsort(SortedRegions, NumProfileRegions, sizeof(unsigned), cmpfunc);
}
...
void Display_Profile(void) {
	...
	row = 0;
	p.Y = ROW_TO_Y(row);
	snprintf(buffer, LCD_MAX_COLS, "%4lu Total Samples        ", profile_samples); 
	...
	for (i=0; i<NumProfileRegions; i++) {
		if (RegionCount[SortedRegions[i]] > 0) {
			p.X = COL_TO_X(0);
			p.Y = ROW_TO_Y(row);
			snprintf(buffer, LCD_MAX_COLS, "%4u %s", RegionCount[SortedRegions[i]], RegionTable[SortedRegions[i]].Name);
			LCD_Text_PrintStr(&p, buffer);
			row++;
			if (row >= LCD_MAX_ROWS) {
	...
	while (1) {
			for (row = 0; row < LCD_MAX_ROWS-1; row++) {
				if ((i < NumProfileRegions) && (RegionCount[i] > 0)) {
					p.X = COL_TO_X(0);
					p.Y = ROW_TO_Y(row);
					snprintf(buffer, LCD_MAX_COLS, "%4u %s", RegionCount[i], RegionTable[i].Name);
					LCD_Text_PrintStr(&p, buffer);
				}
				i++;
				p.X = COL_TO_X(0);
